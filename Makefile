...
econfigure:
	@emcmake cmake . -B build -D CMAKE_BUILD_TYPE=Debug -G $(BACKUP_BUILD_SYSTEM) -DGOON_FULL_MACOS_BUILD=ON -DCMAKE_VERBOSE_MAKEFILE=ON -Dvendored_default=TRUE -DSDL2TTF_VENDORED=TRUE -DFREETYPE_DIR=/usr/lib/x86_64-linux-gnu
# Macos Runner
mconfigure:
...
# Macos Runner Future
xconfigure:
	@cmake . -B build -D CMAKE_BUILD_TYPE=Debug -G $(XCODE_BUILD_SYSTEM) -DGOON_FULL_MACOS_BUILD=ON -DIOS_PLATFORM=OS -Dvendored_default=TRUE -DSDL2TTF_VENDORED=TRUE
